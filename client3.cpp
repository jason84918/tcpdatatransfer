...
int main(int argc,char **argv){
    ...
    for(;;){
        ...
        if(FD_ISSET(0,&rset)){
            ...
            if(v.size()==3&&v[0]=="PUT"){
                if(write(sockfd,buffer,strlen(buffer))<0) perror("write error");
                FILE* fp;
                fp=fopen(v[1].c_str(),"rb");
                if(fp==NULL){
                    char sourceerror[]="source error\n";
                    cout<<sourceerror;
                    if(write(sockfd,sourceerror,strlen(sourceerror))<0) perror("write error");
                    continue;
                }
                char sourcesuccess[]="source success\n";
                //cout<<sourcesuccess;
                if(write(sockfd,sourcesuccess,strlen(sourcesuccess))<0) perror("write error");
                char buffer1[256];
                bzero(buffer1,256);
                ...
                sprintf(length,"%ld\n",filelen);
           		fclose(fp);
           		if(write(sockfd,length,strlen(length))<0) perror("write error");
            }
